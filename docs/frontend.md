...
| `/about` | Who can edit, whether it works offline, where to complain, and — folded shut at the foot — what the server can see. The one screen the app spends on itself, off the quiet line at the foot of the groups list. No pitch: whoever is here already has the app. Two client islands in an otherwise static page — the install offer under "Works offline" (`AboutInstall`, the same `lib/install.ts` state as the nudge on the groups list) and the fold itself (`AboutPrivacy`). Privacy *shows* one stored row rather than asserting anything, so it is only honest while op bodies reach the server sealed ([ADR-0036](decisions/0036-the-server-cannot-read-a-group.md)) and changes in the same commit as that does. The receipt-scan exception is repeated here, but the copy that has to be read is `copy.scan.freeTier`, on the scan screen itself |
