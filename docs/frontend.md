...
| `/about` | What the app is, what the server can see, and where to complain — the one screen it spends on itself, off the quiet line at the foot of the groups list. Prose and two links: a `mailto:` and the source. Its privacy paragraph says what is true today, that op bodies reach the server sealed ([ADR-0036](decisions/0036-the-server-cannot-read-a-group.md)), so it changes in the same commit as that does |
