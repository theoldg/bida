...
| `/join#<groupId>.<secret>` | Invite landing: saves the secret, pulls, then opens the group. A phone that has never said who it is goes on to `/g/claim` — but by `useClaimGate` below, not by this screen, so the same link opened again by someone already in the group just opens it. A fragment with a group id and no secret — and any `/g` screen for a group this phone doesn't hold — shows `KeylessLink` instead of "Bad link": that is the browser bar's address, so it says so and draws the group menu with "Copy invite link" lit. Both failures share its layout and print the link they are about — what was pasted, if it came by **Paste link** (`lib/failed-link.ts`) In an iOS tab, a group this phone hasn't claimed or chosen to keep stops first on **Add to home screen / Continue in Safari** (`asksBeforeJoin`, [ios.md](ios.md#the-design)) |
| `/install[?copied]` | iOS only: why the home-screen app, that it starts empty, and how. Off the iOS tab's banner atop the groups list, or a join's **Add to home screen**, which copies the invite and adds the Paste link step |
