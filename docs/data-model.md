...
- A member is a person in the group, not an account. One with expenses attached
  is tombstoned, never hard-deleted, or the fold references nothing.
...
- A settlement is kept structurally separate from an expense so it never
  pollutes "how much did the trip cost".
...
- **Identity is one row per device**, keyed by the node id that ends every HLC
  that device stamped. Claims are ops
  ([ADR-0011](decisions/0011-identity-changes-are-public.md), superseding 0009).
...
1. Ideal share as an exact rational; 2. floor to minor units; 3. distribute the
remaining units by **largest fractional remainder**, ties broken by a hash of
...
4. return the map plus `remainderAbsorbedBy`.
...
Determinism matters more than fairness: two phones folding the same ops must
produce byte-identical splits or balances diverge. The seeded draw buys fairness
...
on* are different questions with different answers
...
- `resolvePayers()` apportions the stored `baseAmountMinor` at read time so the
  payer side sums to it exactly.
- `paidBy` is kept in step as the largest contributor; a map with one live
  contributor is stored as `null`.
...
producing at most `n−1` transfers — not provably minimal (NP-hard), just good.
Say "simplest way to settle", never "optimal".
