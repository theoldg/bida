...
was ordered. The people are a hairline-ruled list, each line carrying a printed
bill's dotted leader out to its figure — the same reading a quick split ends on
(ADR-0035), which differs only in starting with every row open. Weights and
lines come out of one pass, so a row and the lines under it cannot disagree.
