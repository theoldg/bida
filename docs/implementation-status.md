...
| **Deletable**   | `/delete-my-data` takes an invite link, shows the group it opens, and deletes it from D1 for everybody in it, leaving a tombstone no phone can push past ([frontend.md](frontend.md#deleting-a-group), [sync.md](sync.md#deleting-a-group)). **Its migration, `0003_group_tombstone.sql`, is not applied yet on either environment** ([hosting.md](hosting.md#a-schema-change-from-here-on)) |
