...
| **Versioned** | `major.semi.minor` in the root `package.json`, at the foot of `/about` and the first line of `/diag`. Every push to `dev` deploys, so every push bumps the minor and `pnpm check` insists on it; the major is the owner's alone ([hosting.md](hosting.md#versions)) |
