...
**Status:** Accepted · 2026-08-27 · offline section 2026-08-29
...
- The file list and a content-addressed revision are **stamped in at build
  time** by `apps/web/scripts/precache.mjs`, from what is actually on disk. No
  route list is maintained by hand, and no cache version is bumped by hand — the
...
Cache-first is the point: everything an installed app needs is on the device
under a revision that changes with the build, so a launch and every tap after it
paint without waiting on the network.

## Consequences

- No server runtime, no SSR/CSR split, no hydration mismatch, instant edge loads.
...
- `scripts/offline-check.mjs` is the regression test: it drives the real export
  in Chromium, cuts the network, walks fourteen screens including a save, then
  installs a deploy over a half-dead network. Run it after touching `sw.js`.

## Rejected

- **`@opennextjs/cloudflare`** — full Next.js feature set, at the cost of a
  Worker invocation per request, a larger bundle and an adapter between us and
  the framework.
