...
primitive was ever copied in: the markup and class names were already finished
and specific, so copying a primitive and re-skinning it would have been strictly
more work. Separately, the last screens still used the browser's own furniture —
`prompt()` and `confirm()` for members, drafts and unlisted currencies;
`<select>` for every picker. The owner: *"make all selectors into a custom
...
- **It is a real `<dialog>` opened with `showModal()`.** Focus, Escape and the
  ...
  hairline card and the buttons. What they look like and how their rows read is
...
- **A headless dialog library, or our own focus trap** — `<dialog>` deleted
  fifteen lines of Tab-wrapping and added real inertness for a screen reader.
- **A `<select>` styled to look like ours** — only the closed state is stylable,
  which is the half that was already fine. **Keeping `confirm()` for the small
  ones** — two dialog languages, and the small one is the one that reads as the
  browser interrupting.
