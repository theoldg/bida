...
editable."* The app had two of the three and called neither by the right name;
money coming *in* had nowhere to go but a negative-shaped lie in a description.
...
`payers`, the same `split` and the same validation; the sign is applied in
exactly one place, `computeBalances`, where receivers are debited and sharers
credited. Nothing else in `packages/core` branches on it.
...
always meant; present, it wins, and `paidBy` is maintained as the largest
contributor so older clients and list rows still read. `resolvePayers` apportions
`baseAmountMinor` with the same seeded largest-remainder rule a `shares` split
uses, so the payer side sums to the base total exactly.
...
removing the variant would break the one thing the op log promises, that old ops
...
- Adding an entry is one screen and one **Save** for every ordinary case, at the
  cost of a form that scrolls further with a five-person group.

## Rejected

- **A separate `income` entity** — it would duplicate `payers`, `split`,
  `receiptItems` and every validation in core for two things that differ by a
  sign.
- **A negative `amountMinor`** — every piece of split and payer arithmetic
  assumes a positive total, so the cost lands as sign handling in a dozen places
  instead of one.
...
- **Splitting a co-paid bill into several single-payer entries** — two rows for
  ...
  the amount.
- **A speed-dial FAB offering three targets** — two taps for the common case,
  answering a question the form is about to ask anyway.
